...
app = Flask(__name__)
app.config['SECRET_KEY'] = os.environ.get('SECRET_KEY', 'dev_tinder_super_secret_hackathon_key')
...
def calculate_compatibility(user_a, user_b):
    ...
    def get_categories(skills):
        cats = set()
        if skills.intersection(frontend_skills): cats.add('frontend')
        if skills.intersection(backend_skills): cats.add('backend')
        if skills.intersection(ai_skills): cats.add('ai')
        if skills.intersection(security_skills): cats.add('security')
        return cats
    ...
    interests = ['AI', 'Web Development', 'Cybersecurity', 'Open Source', 'Blockchain']
    interests_a = {i for i in interests if i.lower() in (user_a.bio or '').lower() or i.lower() in user_a.skills.lower()}
    interests_b = {i for i in interests if i.lower() in (user_b.bio or '').lower() or i.lower() in user_b.skills.lower()}
    if not interests_a and not interests_b:
        ...
        interest_score = (len(intersect_int) / len(union_int)) * 20 if union_int else 10
    ...
    total_score = skill_sim_score + complementarity_score + interest_score + avail_score + exp_score
...
def generate_project_idea_mock(skills, theme, difficulty, retrieved=None):
    ...
    title_themes = {
        'AI': [f"Smart {theme} Matcher", "DocuSynth AI", "OmniVision Health", "EcoAgent AI"],
        'Web Development': ["Dev Tinder", "TaskStream Flow", "LinkSphere", "HackerRank Analytics"],
        'Cybersecurity': ["Sentinel Shield AI", "ZeroTrust Portal", "AuthVault", "PhishCatcher Pro"],
        'Open Source': ["GitCollab Dashboard", "OSS ContribFinder", "LibCheck Security", "FOSS Tracker"],
        'Blockchain': ["SmartEscrow Ledger", "NFT Certificate Hub", "DAO Voting Mesh", "CryptoSafe Vault"]
    }
    
    selected_theme = theme if theme in title_themes else 'Web Development'
    title = random.choice(title_themes[selected_theme])
    
    problem = f"Hackathon teams struggle to validate and build MVP solutions in {theme} domains because of high friction in coordination and a lack of integrated tools."
    audience = "Developers, Hackathon judges, startup founders, and technical recruiters."
    features = ["Real-time dashboard", "AI-powered recommendations", "Collaborative space", "Automatic code generation templates"]
    ...
    monetization = "Freemium model with premium developer templates, custom analytics packages for companies, and referral fees."
    pitch = f"Devs! The future of hackathons is here. {title} solves the core friction points by introducing AI matching with immediate sandbox workspace setups."
    script = f"[Presenter]: Hi judges! We are presenting {title}. First, the user creates an account, then invites members. We click generate, and boom - immediate MVP workspace config. Thank you!"
    success_score = random.randint(75, 95)

    inspired_by = ", ".join([r['title'] for r in retrieved]) if retrieved else None
...
def generate_project_idea_ai(skills, theme, difficulty):
    ...
    if not GEMINI_API_KEY:
        return generate_project_idea_mock(skills, theme, difficulty, retrieved=retrieved)
    ...
    try:
        ...
        Provide the response in raw JSON format with the following keys. Return ONLY the JSON object, do not wrap in markdown tags or write conversational text:
        {{
...
def forgot_password():
    if request.method == 'POST':
        email = request.form.get('email')
        flash(f"If the email {email} exists in our database, a password reset link has been sent.", "info")
...
def dashboard():
    ...
    for u in potential_users:
        ...
        match_exists = Match.query.filter(
            ((Match.user_one_id == current_user.id) & (Match.user_two_id == u.id)) |
            ((Match.user_one_id == u.id) & (Match.user_two_id == current_user.id))
        ).first()
        if not match_exists:
            score = calculate_compatibility(current_user, u)
            suggestions.append({'user': u, 'score': score})
            
    suggestions = sorted(suggestions, key=lambda x: x['score'], reverse=True)[:4]
    
    # Active Matches Count
    active_matches = Match.query.filter(
        ((Match.user_one_id == current_user.id) | (Match.user_two_id == current_user.id)) &
        (Match.status == 'connected')
    ).all()
    
    # Team
    membership = TeamMember.query.filter_by(user_id=current_user.id, status='active').first()
...
def profile():
    if request.method == 'POST':
        ...
        current_user.grad_year = int(request.form.get('grad_year', 2026) or 2026)
        ...
        current_user.hackathons_attended = int(request.form.get('hackathons_attended', 0) or 0)
        current_user.hackathons_won = int(request.form.get('hackathons_won', 0) or 0)
        ...
        if avatar_style:
            current_user.profile_image = f"https://api.dicebear.com/7.x/{avatar_style}/svg?seed={current_user.username}"
...
@login_required
def github_analyzer():
    """Mock-real GitHub profile data fetcher. Falls back to mock UI data if API is rate limited or user doesn't have github URL."""
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            repos = []
            languages = {}
            total_contributions = random.randint(100, 500) # Mock contributions as GitHub API doesn't expose it simply without graphQL
...
def swipe():
    ...
    if action != 'pass' and opposite_swipe:
        ...
        n1 = Notification(
            ...
            title="It's a Match! 🎉",
            message=f"You connected with {target_user.full_name or target_user.username} ({score}% match)!",
            type='match',
            link=url_for('chat_user', chat_user_id=target_user_id)
        )
        n2 = Notification(
            user_id=target_user_id,
            title="It's a Match! 🎉",
            message=f"You connected with {current_user.full_name or current_user.username} ({score}% match)!",
            type='match',
...
def team():
    ...
    invites = TeamMember.query.filter_by(user_id=current_user.id, status='invited').all()
    ...
    if team:
        ...
        roles_strength = {'frontend': 0, 'backend': 0, 'design': 0, 'ai': 0, 'security': 0}
        ...
        for m in team.members:
            ...
            for s in user_skills:
                if s in frontend_skills: roles_strength['frontend'] += 20
                if s in backend_skills: roles_strength['backend'] += 20
                if s in ai_skills: roles_strength['ai'] += 30
                if s in security_skills: roles_strength['security'] += 40
                
        # Caps strengths at 100
        for k in roles_strength:
            roles_strength[k] = min(roles_strength[k] + 20, 100) if roles_strength[k] > 0 else 10 # small default baseline if empty
        ...
        if roles_strength['frontend'] < 50:
            health_suggestions.append("Add a Frontend developer or UI/UX Designer to enhance visual aesthetic.")
        if roles_strength['backend'] < 50:
            health_suggestions.append("Incorporate Python/Flask backend expertise to handle databases securely.")
        if roles_strength['design'] < 40:
            health_suggestions.append("Team lacks UI/UX design strength. Suggest recruiting a Figma designer.")
        if roles_strength['ai'] < 30:
            health_suggestions.append("AI and ML capabilities are low. Add a member skilled in NLP or Gemini API integrations.")
        if roles_strength['security'] < 30:
            health_suggestions.append("Ensure your app is secure. Recruit a Cybersecurity specialist.")
            
        if len(health_suggestions) == 0:
            health_suggestions.append("Excellent team balance! You are fully equipped to win the hackathon.")
...
def create_team():
    ...
    old_membership = TeamMember.query.filter_by(user_id=current_user.id, status='active').first()
    if old_membership:
...
def invite_member():
    ...
    already_member = TeamMember.query.filter_by(team_id=team.id, user_id=target_user_id).first()
    if already_member:
    ...
    notif = Notification(
        ...
        message=f"You have been invited to join team '{team.name}' as a {role}!",
        type='invite',
        link=url_for('team')
    )
...
def respond_invite(invite_id, action):
    ...
    if action == 'accept':
        ...
        n = Notification(
            ...
            title="Member Joined! 👥",
            message=f"{current_user.full_name or current_user.username} accepted your invitation to join '{invite.team.name}'!",
            type='invite',
            link=url_for('team')
        )
        db.session.add(n)
...
def remove_member(member_id):
    ...
    flash(f"Removed member from team.", 'success')
    return redirect(url_for('team'))
...
def one_click_team_generator():
    ...
    in_teams = db.session.query(TeamMember.user_id).filter_by(status='active').subquery()
    available_users = User.query.filter(User.id != current_user.id).filter(~User.id.in_(in_teams)).all()
    
    if not available_users:
        flash("No available single users at the moment to construct a team.", "warning")
        return redirect(url_for('team'))
    ...
    my_skills = set(current_user.get_skills_list())
    ...
    team_candidates = sorted(team_candidates, key=lambda x: x['score'], reverse=True)
    
    # Create the team
    team_name = f"Autobuild Team {random.randint(100, 999)}"
    new_team = Team(name=team_name, description="AI-generated balanced hackathon team.", creator_id=current_user.id)
    db.session.add(new_team)
    db.session.flush()
    
    # Join creator
    db.session.add(TeamMember(team_id=new_team.id, user_id=current_user.id, role='Team Lead (Fullstack)', status='active'))
    ...
    for candidate in team_candidates[:2]:
        ...
        # Add to team
        member = TeamMember(team_id=new_team.id, user_id=user.id, role=role, status='active')
        db.session.add(member)
    ...
    db.session.commit()
    flash(f"Auto-generated team '{team_name}' successfully with {added + 1} balanced members!", 'success')
    return redirect(url_for('team'))
...
def project_generator():
    ...
    saved_projects = []
    if team:
        saved_projects = Project.query.filter_by(team_id=team.id).order_by(Project.created_at.desc()).all()
    else:
        saved_projects = Project.query.filter_by(creator_id=current_user.id).order_by(Project.created_at.desc()).all()
        
    return render_template('project_generator.html', team=team, saved_projects=saved_projects)
...
def generate_project():
    ...
    flash(f"Generated new AI Project recommendation: '{project.title}'!", 'success')
...
def chat():
    ...
    matches_one = Match.query.filter_by(user_one_id=current_user.id, status='connected').all()
    matches_two = Match.query.filter_by(user_two_id=current_user.id, status='connected').all()
    
    chat_users = []
    for m in matches_one:
        user = User.query.get(m.user_two_id)
        if user: chat_users.append(user)
    ...
    return render_template('chat.html', chat_users=chat_users, active_chat_user=None, team=team)
...
def chat_user(chat_user_id):
    ...
    Message.query.filter_by(sender_id=chat_user_id, receiver_id=current_user.id, is_read=False).update({Message.is_read: True})
    db.session.commit()
    
    membership = TeamMember.query.filter_by(user_id=current_user.id, status='active').first()
    team = membership.team if membership else None
    
    return render_template('chat.html', chat_users=chat_users, active_chat_user=active_chat_user, team=team, is_team_chat=False)

@app.route('/chat/team/<int:team_id>')
@login_required
def chat_team(team_id):
    # Verify user is in this team
    member = TeamMember.query.filter_by(team_id=team_id, user_id=current_user.id, status='active').first_or_404()
    ...
    return render_template('chat.html', chat_users=chat_users, active_chat_user=None, team=member.team, is_team_chat=True)
...
def send_message():
    ...
    if receiver_id:
        ...
        n_exists = Notification.query.filter_by(user_id=receiver_id, type='chat', is_read=False).first()
        if not n_exists:
            n = Notification(
                user_id=receiver_id,
...
def get_messages():
    ...
    if team_id:
        ...
        query = query.filter(
            ((Message.sender_id == current_user.id) & (Message.receiver_id == int(receiver_id))) |
            ((Message.sender_id == int(receiver_id)) & (Message.receiver_id == current_user.id))
        )
    else:
        return jsonify([])
        
    messages = query.filter(Message.id > last_id).order_by(Message.created_at.asc()).all()
    
    # Mark incoming read
    if receiver_id:
        Message.query.filter_by(sender_id=int(receiver_id), receiver_id=current_user.id, is_read=False).update({Message.is_read: True})
        db.session.commit()
...
def leaderboard():
    ...
    users_ranking = db.session.query(
        User, db.func.sum(Rating.activity_score).label('total_activity')
    ).join(Rating, User.id == Rating.user_id).group_by(User.id).order_by(db.desc('total_activity')).limit(10).all()
    ...
    for t in teams:
        strengths = {'frontend': 0, 'backend': 0, 'design': 0, 'ai': 0, 'security': 0}
        for m in t.members:
            if m.status == 'active':
                user_skills = set(m.user.get_skills_list())
                for s in user_skills:
                    if s in frontend_skills: strengths['frontend'] += 20
                    if s in backend_skills: strengths['backend'] += 20
                    if s in ai_skills: strengths['ai'] += 30
                    if s in security_skills: strengths['security'] += 40
        for k in strengths:
            strengths[k] = min(strengths[k] + 20, 100) if strengths[k] > 0 else 10
    ...
    ranked_teams = sorted(ranked_teams, key=lambda x: x['score'], reverse=True)[:5]
    
    return render_template('leaderboard.html', ranked_users=ranked_users, ranked_teams=ranked_teams)
...
def hackathons():
    ...
    return render_template('dashboard.html', hack_discover_only=True, hacks=hacks, active_filter=cat_filter)
...
def migrate_new_columns():
    ...
    with db.engine.connect() as conn:
        existing_cols = [row[1] for row in conn.execute(text("PRAGMA table_info(projects)"))]
        if 'inspired_by' not in existing_cols:
            conn.execute(text("ALTER TABLE projects ADD COLUMN inspired_by TEXT"))
        if 'differentiation' not in existing_cols:
            conn.execute(text("ALTER TABLE projects ADD COLUMN differentiation TEXT"))
        conn.commit()
...
def seed_database():
    ...
    users_data = [
        {
            ...
            'hackathons_won': 4,
            'bio': 'PhD AI student interested in LLMs, Prompt Engineering, and Natural Language Processing. Looking for frontend experts to team up with!',
            'profile_image': 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=150&h=150&fit=crop',
            'github': 'priya-ai', 'linkedin': 'in/priyasharma'
        },
        {
            'username': 'alex_dev',
            ...
            'hackathons_won': 2,
            'bio': 'React Native and Tailwind absolute wizard. I construct interactive animations and stunning dashboard aesthetics.',
            'profile_image': 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=150&h=150&fit=crop',
            'github': 'alex-johnson-dev', 'linkedin': 'in/alexjohnson'
        },
        {
            'username': 'kabir_sec',
            ...
            'hackathons_won': 5,
            'bio': 'Vulnerability analyst and penetration tester. Love securing REST APIs and working on Blockchain networks.',
            'profile_image': 'https://images.unsplash.com/photo-1522075469751-3a6694fb2f61?w=150&h=150&fit=crop',
            'github': 'kabirsecur', 'linkedin': 'in/kabirmehta'
        },
        {
            'username': 'sarah_uiux',
            ...
            'hackathons_won': 1,
            'bio': 'Passionate designer. I create detailed Figma prototype boards, wireframes, and design systems for web apps.',
            'profile_image': 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=150&h=150&fit=crop',
            'github': 'sarahdesign', 'linkedin': 'in/sarahdesign'
        },
        {
            ...
            'hackathons_won': 0,
            'bio': 'Backend builder. I design secure endpoints, write fast SQL queries, and manage Docker pipelines.',
            'profile_image': 'https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=150&h=150&fit=crop',
            'github': 'rohitsendev', 'linkedin': 'in/rohitsen'
        }
    ]
    ...
    # 3. Dummy Teams
    t1 = Team(name='Neural Ninjas', description='Building AI agents to map evolutionary conservation data.', creator_id=users_created[0].id)
    t2 = Team(name='ZeroTrust Crusaders', description='Implementing zero-trust architecture in educational APIs.', creator_id=users_created[2].id)
    db.session.add(t1)
    db.session.add(t2)
    db.session.flush()
    
    # 4. Team Members
    db.session.add(TeamMember(team_id=t1.id, user_id=users_created[0].id, role='AI Specialist', status='active'))
    db.session.add(TeamMember(team_id=t1.id, user_id=users_created[1].id, role='UI UX Designer', status='active'))
    
    db.session.add(TeamMember(team_id=t2.id, user_id=users_created[2].id, role='Security Lead', status='active'))
    db.session.add(TeamMember(team_id=t2.id, user_id=users_created[4].id, role='Backend Architect', status='active'))
    
    # 5. Activity Ratings (for Leaderboard)
    for u in users_created:
        db.session.add(Rating(user_id=u.id, activity_score=random.randint(50, 200)))
        
    # 6. Matches
    # Priya and Kabir connect
    db.session.add(Match(user_one_id=users_created[0].id, user_two_id=users_created[2].id, status='connected', action_by_id=users_created[0].id, match_score=78))
    # Priya and Sarah connect
    db.session.add(Match(user_one_id=users_created[0].id, user_two_id=users_created[3].id, status='connected', action_by_id=users_created[3].id, match_score=85))
    
    # Priya messages Sarah
    db.session.add(Message(sender_id=users_created[3].id, receiver_id=users_created[0].id, content="Hi Priya! I saw your post. I would love to design the mock UI panels for your AI platform!"))
    db.session.add(Message(sender_id=users_created[0].id, receiver_id=users_created[3].id, content="Awesome Sarah! I am looking exactly for design profiles. Let's discuss on team tab."))
